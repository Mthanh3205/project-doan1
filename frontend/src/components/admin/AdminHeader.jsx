import { Menu, Bell, Search, Mail, X, Star, Eye, EyeOff, Trash2 } from 'lucide-react';
...
export default function AdminHeader({ setIsSidebarOpen }) {
  ...
  const handleNotificationClick = async (noti) => {
    if (noti.type === 'feedback' && noti.reference_id) {
      try {
        ...
        // 1. Gọi API lấy chi tiết bài đánh giá
        const res = await fetch(
        ...
        if (res.ok) {
          ...
          setSelectedReview(reviewData); // Mở Modal lên
          setShowNoti(false); // Đóng dropdown thông báo
        } else {
          toast.error('Đánh giá này có thể đã bị xóa.');
        }

        // 2. Đánh dấu thông báo này là đã đọc (Optional - Cần API update single noti nếu muốn)
      } catch (error) {
        console.error(error);
  ...
  // --- CÁC HÀM HÀNH ĐỘNG TRONG MODAL (Ẩn/Xóa) ---
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSelectedReview(null); // Đóng modal
        fetchNotifications(); // Load lại thông báo (có thể bài đó sẽ mất đi)
      }
    } catch (error) {
      toast.error('Lỗi xóa');
    }
  ...
  return (
    <>
                ...
                <div className="max-h-64 overflow-y-auto py-1">
                  {notifications.length === 0 ? (
                    ...
                    notifications.map((n) => (
                      <div
                        key={n.id}
                        onClick={() => handleNotificationClick(n)} // Click vào để xem chi tiết
      ...
      {/* --- POPUP MODAL CHI TIẾT (Copy từ ReviewsManager sang) --- */}
      {selectedReview && (
        <div
